% fmridisplay Data class for underlay + activation map montages and plots.
%
...
%
%     obj = fmridisplay;
%     obj = fmridisplay('overlay', overlay_filename);
%     obj = fmridisplay('montage', 'addblobs', cl);
%
% :Optional Inputs:
%
%   **'overlay':**
%        Followed by a filename for the underlay image. Default:
%        which('fmriprep20_template.nii.gz').
%
%   **'montage':**
%        If present, calls montage() on the constructed object and
%        passes through remaining varargin.
%
%   **'addblobs':**
%        Followed by a clusters/region structure to add as blobs.
%
% :Outputs:
%
%   **obj:**
%        An fmridisplay object with .overlay, .SPACE,
%        .activation_maps, .montage, .surface, .orthviews, .history,
%        and .additional_info fields populated.
%
% :Examples:
% ::
%
...
%            obj.overlay = which('spm152.nii');
            obj.overlay = which('fmriprep20_template.nii.gz');
