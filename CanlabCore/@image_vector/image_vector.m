...
% image_vector An object class for storage and manipulation of neuroimaging data.
%
% image_vector stores brain image data in a flat (2-D) voxels x images
% matrix together with the meta-data needed to round-trip back to 3-D
% image space. It is the superclass of fmri_data, statistic_image, and
% atlas; you will rarely create image_vector objects directly. Use one
% of those subclasses instead.
%
...
% atlas
%
% These object types have additional analysis methods and properties,
% and are most often used. You will rarely create an image_vector object
% directly. Typically you would create instances of one of the classes
% above.
%
% :Usage:
% ::
%
%     obj = image_vector(filename_or_fields)
%     obj = image_vector('fullpath', fname, 'history', {'loaded from file.'})
%
% :Inputs:
%
%   **filename_or_fields:**
%        Either a Nifti / Analyze filename to read, a structure whose
%        fields will be parsed into the object, or a sequence of
%        key/value pairs assigning fields directly. See the
%        constructor body and Examples below for usage.
%
% :Construction:
%
%   As with any neuroimaging-data object class in this toolbox
%   (image_vector, fmri_data, statistic_image, atlas), you can create
%   an object by specifying the names of a set of images you want to
%   load in and passing them into the function with the same name as
%   the object class (e.g., fmri_data). This will create an object
%   with image data and image space information attached.
%
...
%
%   You can also manually construct an object by entering its
%   constituent fields in key, value pairs. For the object to be valid,
%   these must match one another in size/space/etc, and you may get
%   errors later if they do not match.
%
%   EXAMPLE: Create an empty image_vector object and assign the image
%   in fname to the .fullpath property. The image_vector constructor
%   will read in the image stored in fname.
%
%       obj = image_vector('fullpath', fname, 'history', {'loaded from file.'});
%
%   EXAMPLE: The code below first loads a sample atlas object and then
%   creates an image_vector object from its fields. Then, we re-cast it
%   as an fmri_data object, which is generally more useful and has more
%   associated methods.
%
...
%
%   You can also enter a structure, and image_vector will parse input
%   fields from the structure. Here we use a structure to define a
%   custom .dat field that overwrites the one read from the file in fname:
%
...
%
%   Properties are data fields associated with an object. Type the name
%   of an object (class instance) you create to see its properties, and
%   a link to its methods (things you can run specifically with this
%   object type). For example: After creating an fmri_data object
%   called fmri_dat, as above, type fmri_dat to see its properties.
%
%   There are many other methods that you can apply to fmri_data
%   objects to do different things.
%
...
%     - write(fmri_dat) writes an image file to disk (careful not to
%       overwrite by accident!)
%     - regress(fmri_dat) runs multiple regression
%     - predict(fmri_dat) runs cross-validated machine
