function [preprocessed_dat, roi_val] = canlab_connectivity_preproc(dat, varargin)
...
%                           canlab_create_wm_ventricle_masks.m)
%               example: 'vw', 'dat_dir', subject_dir, 'raw'
%
...
%               option should be followed by masks (in char or cell arrays).
...
if do_preproc
    ...
    tic;
    fprintf('Canlab_conn_preproc: starting preprocessing ... \n');
    ...
    %% linear trend
    if do_linear, comb_R(:,end+1) = scale((1:size(comb_R,1))'); end
    
    % (optional) global signal % not implemented
    
    %% get residuals 
    
    X = [comb_R ones(size(comb_R,1),1)];
    ...
    if do_windsorize, dat = windsorize(dat, k_std); end
    
    %% pass filter (low and high): low pass, high pass, banpass
...
function obj = windsorize(obj, k)
    ...
    obj.history{end+1} = sprintf('\n--- Windsorized data matrix to %d STD; adjusted %3.0f values, %3.1f%% of values', k, nbad, percbad);
