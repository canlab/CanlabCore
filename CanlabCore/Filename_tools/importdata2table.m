function [data_struct, data_table, raw] = importdata2table(excelfilename, has_header_row)
% Reads a text input file into a data structure and corresponding table object
% Note: see Matlab's readtable.m, which may be better (or at least complementary)
%
% :Usage:
% ::
%
%    [data_struct, data_table, raw] = importdata2table(excelfilename, has_header_row [1 or 0])
%
...
%
% :See also:
% readtable, importdata, read_database, read_database2, read_edat_output_2008, read_physio_data
%
% ..
%    Programmers' notes:
...
raw = importdata(excelfilename); % read the original data

[data_struct, data_table] = process_sheet(raw.data, raw.textdata, has_header_row);
...
function [data_struct, data_table] = process_sheet(data, textdata, has_header_row)
...
for i = 1:length(varnames)
    ...
    if istext
        ...
        % Handle various cases read as text, including mixed-format
        % numbers/text and missing/empty values
        mytext = process_data_column(mytext);
