function canlab_glm_subject_levels_old(dsgnarg, varargin)
...
if ~isempty(OPTS.parallel_dream)
    ...
    if OPTS.parallel_local
        parfor s = 1:numel(DSGN.subjects)
            ...
            [modelstatus(s) constatus(s) linkstatus(s)] = canlab_glm_subject_levels_run1subject_old(wd,s); % lukasvo changed to _old version of function which works with LaBGAS firstlevel code
        end
    else
        for s = 1:numel(DSGN.subjects)
            ...
            [modelstatus(s) constatus(s) linkstatus(s)] = canlab_glm_subject_levels_run1subject_old(wd,s);
        end
    end        
