function [modelstatus constatus linkstatus] = canlab_glm_subject_levels_run1subject_old(wd, s)
...
%         vol_cnts = vol_cnts(vol_cnts(~cellfun(@isempty,onsets) & ~cellfun(@isempty,durations)) > 0); LVO revert Bogdan's improvement since it breaks LaBGAS firstlevel code
...
% emptyons = find(~ismember(1:length(oldruns), find(~cellfun(@isempty, oldonsets)))); % LVO reverted Bogdan's improvement again
...
%     if ~any(catruns == r) && ~ismember(r, emptyruns) % LVO again reverting Bogdan's improvement
