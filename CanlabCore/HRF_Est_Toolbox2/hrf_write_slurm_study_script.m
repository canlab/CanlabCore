...
fprintf(fid, 'try\n');
fprintf(fid, '    manifest = readtable(manifest_file, ''TextType'', ''string'', ''VariableNamingRule'', ''preserve'');\n');
fprintf(fid, 'catch\n');
...
fprintf(fid, 'if width(manifest) < 6, error(''Manifest must have at least 6 columns: index, subject, fmri_file, events_file, output_prefix, output_mat.''); end\n');
fprintf(fid, 'get_col = @(col) char(string(manifest{task_id, col}));\n');
fprintf(fid, 'subject = get_col(2);\n');
fprintf(fid, 'fmri_file = strrep(get_col(3), ''\\'', filesep);\n');
fprintf(fid, 'events_file = strrep(get_col(4), ''\\'', filesep);\n');
fprintf(fid, 'output_prefix = strrep(get_col(5), ''\\'', filesep);\n');
fprintf(fid, 'output_mat = strrep(get_col(6), ''\\'', filesep);\n');
fprintf(fid, 'fprintf(''Running HRF task %%d/%%d: %%s\\n'', task_id, height(manifest), subject);\n');
