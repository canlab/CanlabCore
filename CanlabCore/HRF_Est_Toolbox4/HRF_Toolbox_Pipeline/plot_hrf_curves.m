function fig = plot_hrf_curves(input_table, varargin)
...
% Usage
% -----
%   fig = plot_hrf_curves(input_table_lf);                % defaults
%   fig = plot_hrf_curves(input_table_lf, ...
%             'Model', 'sfir', ...
...
%   'ShareYAxis'  - share one y-scale across all panels. Default is
%                   source-aware: true for 'atlas' (amplitudes comparable
%                   across regions), false for 'signature'/'imageset'
%                   (pattern-similarity scales differ wildly, so each panel
%                   auto-scales). Pass true/false to override.
%   'ShowSignificance' - true (default) marks lags where the across-subject
%                   mean differs from 0 at p < Alpha (one-sample t per lag):
...
% ShareYAxis default is source-aware: atlas regions share a y-axis (amplitudes
% are comparable across regions), but signatures/networks have wildly
% different pattern-similarity scales, so each panel auto-scales unless the
% user overrides. Honor an explicit value if given.
if isempty(opts.ShareYAxis)
    opts.ShareYAxis = strcmp(source_family, 'atlas');
end
if ~ismember(source_family, {'atlas', 'signature', 'imageset'})
    error('plot_hrf_curves:UnknownSource', ...
...
if height(long) == 0
    error('plot_hrf_curves:NoMatchingConditions', ...
...
if isempty(top_regions)
    ...
    if ~isempty(opts.Regions)
        ...
        error('plot_hrf_curves:NoMatchingRegions', ...
        ...
        error('plot_hrf_curves:NoRegions', ...
...
if ~any(strcmp(file_col, v))
    error('plot_hrf_curves:NoFileColumn', ...
...
if verbose
    fprintf('plot_hrf_curves: input_table has %d rows; source=''%s'', model=''%s'', object=''%s''\n', ...
...
% Per-lag one-sample t-test of the across-subject mean vs 0, so the plot
% can mark lags where the curve is significantly different from baseline.
% df = n_subjects - 1; p is two-tailed. n < 2 -> NaN (no test possible).
tstat = pooled.mean ./ pooled.sem;
dfe = max(pooled.n - 1, 0);
pval = NaN(height(pooled), 1);
ok = pooled.n >= 2 & isfinite(tstat) & pooled.sem > 0;
pval(ok) = 2 * (1 - tcdf(abs(tstat(ok)), dfe(ok)));
pooled.t = tstat;
pooled.p = pval;
end
...
% Shared y-range across panels (only used when ShareYAxis is true). The
% significance markers sit slightly above the band, so leave headroom.
if share_y
    [gy_min, gy_max] = local_panel_yrange(pooled, true(height(pooled), 1), opts.ErrorBand);
    gy_pad = 0.08 * (gy_max - gy_min + eps);
end
...
for r = 1:n
    ...
    % Per-panel y-range when not sharing, so wildly different scales
    % (e.g. across signatures) each fill their own panel.
    if share_y
        y_lo = gy_min - gy_pad; y_hi = gy_max + gy_pad;
    else
        region_mask = pooled.region == string(top_regions{r});
        [py_min, py_max] = local_panel_yrange(pooled, region_mask, opts.ErrorBand);
        py_pad = 0.08 * (py_max - py_min + eps);
        y_lo = py_min - py_pad; y_hi = py_max + py_pad;
    end
    yspan = y_hi - y_lo;
    ...
    for k = 1:height(series_keys)
        ...
        % lag (the onset) so "when does it become significant" is visible.
        if show_sig && any(strcmp('p', sub.Properties.VariableNames))
            sig = sub.p < alpha & isfinite(sub.p);
            if any(sig)
...
if isempty(char(opts.Title))
    unit_noun = local_unit_noun(char(opts.Source));
    extra = '';
    if logical(opts.ShowSignificance)
        extra = sprintf('; dots p<%.3g vs 0, ring=onset', opts.Alpha);
    end
    tstr = sprintf('Top %d %s  (model=%s, object=%s, %s ± %s%s)', ...
        n, unit_noun, char(opts.Model), char(opts.Object), ...
        char(opts.Source), char(opts.ErrorBand), extra);
else
...
function noun = local_unit_noun(family)
switch lower(family)
    case 'signature', noun = 'signatures';
    case 'imageset',  noun = 'networks';
    otherwise,        noun = 'atlas regions';
end
end


function [y_min, y_max] = local_panel_yrange(pooled, mask, band)
% Min/max of the (mean ± error) band over the masked rows.
y_min = Inf; y_max = -Inf;
lo = pooled.mean(mask) - local_err(pooled, mask, band);
hi = pooled.mean(mask) + local_err(pooled, mask, band);
lo = lo(isfinite(lo));
hi = hi(isfinite(hi));
if ~isempty(lo), y_min = min(lo); end
if ~isempty(hi), y_max = max(hi); end
