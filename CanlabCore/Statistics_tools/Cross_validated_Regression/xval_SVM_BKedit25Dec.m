...
%         [Sr.trIdx, Sr.teIdx] = xval_stratified_holdout_leave_whole_subject_out(Sr.Y, Sr.id, 'doverbose', false, 'doplot', false);
    ...
    % S.crossval_accuracy = mean(S.crossval_accuracy_repeat);
    % S.classification_d_singleinterval = mean(S.classification_d_singleinterval_repeat);
    
    % S.crossval_accuracy_within = mean(S.crossval_accuracy_within_repeat);
    % S.classification_d_within = mean(S.classification_d_within_repeat);
...
p.addParameter('dorepeats', 3, valfcn_number);
p.addParameter('dobootstrap', true, valfcn_logical);
p.addParameter('nboot', 10, valfcn_number);
