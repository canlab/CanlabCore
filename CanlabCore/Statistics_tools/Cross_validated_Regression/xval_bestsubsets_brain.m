function pmodel_obj = xval_bestsubsets_brain(my_outcomes, imgs, varargin)
    %pmodel_obj = xval_bestsubsets_brain(my_outcomes, imgs, varargin)
    %
    % PCA-bestsubsets based prediction on a set of brain images
    % Tor Wager, Sept. 2009
    %
    %
    % Optional inputs:
    ...
    % Example
    % -----------------------------------------------------------------------
    % mkdir bestsubsets_xvalidation_frontal
    % cd bestsubsets_xvalidation_frontal/
    ...
    % STATS_FINAL = xval_bestsubsets_brain(my_outcomes, imgs, 'mask', mask, 'covs', covs, 'reversecolors', 'skipsurface', 'skipnonparam');
    %
    % or
    %
    % STATS_FINAL = xval_bestsubsets_brain(my_outcomes, imgs, 'mask', mask, 'covs', covs, 'reversecolors');
    ...
    if ~isempty(covs)
        ...
        if dosave, create_figure('fitplot', 2, 2, 1); scn_export_papersetup(400); saveas(gcf, 'bestsubsets_xval_brain_fits_covs_only.png'); end
    end
    ...
    if ~isempty(covs)
        disp('==================================')
        disp('Data only: bestsubsets')
        STATS_FINAL.data = xval_regression_multisubject('bestsubsets', my_outcomes, dat, 'pca', 'ndims', ndims, plsstr, dochoose_regparams_str, 'holdout_method', holdout_method);
    else
        STATS_FINAL.data = 'see .full_model';
    end

    if dosave, create_figure('fitplot', 2, 2, 1); scn_export_papersetup(400); saveas(gcf, 'bestsubsets_xval_brain_fits_brain_only.png'); end

    % Run it: combined
    % -------------------------------------------------------------------------

    % My best guess about what will work: bestsubsets, with as many dims retained as possible
    % * NOTE: all of the main inputs should be cell arrays
    STATS_FINAL.full_model = [];
    if ~isempty(covs)
        disp('==================================')
        disp('Data plus covariates full model: bestsubsets')
        STATS_FINAL.full_model = xval_regression_multisubject('bestsubsets', my_outcomes, dat, 'pca', 'ndims', ndims, 'covs', covs, plsstr, dochoose_regparams_str, 'holdout_method', holdout_method);
    else
        disp('Data only full model: bestsubsets')
        disp('==================================')
        STATS_FINAL.full_model = xval_regression_multisubject('bestsubsets', my_outcomes, dat, 'pca', 'ndims', ndims, plsstr, dochoose_regparams_str, 'holdout_method', holdout_method);
    end

    if dosave
        save STATS_xval_output STATS_FINAL
        create_figure('fitplot', 2, 2, 1); scn_export_papersetup(400); saveas(gcf, 'bestsubsets_xval_brain_fits_full_model.png');
    end
    ...
    ylabel('Outcome');
    title('bestsubsets cross-validation');
    ...
    if dosurface
        STATS_VOXWEIGHTS = xval_regression_multisubject_bootstrapweightmap('bestsubsets', my_outcomes, dat, maskInfo, varargin, 'pca', 'ndims', ndims, 'covs', covs, plsstr);
        save STATS_VOXWEIGHTS STATS_VOXWEIGHTS
    end
    
% % %     %% Get mean bestsubsets betas
% % %     % -------------------------------------------------------------------------
% % % 
% % %     bonf_thresh = norminv(1 - .025 ./ size(dat{1}, 2));
% % % 
% % %     mystd = std(STATS_FINAL.full_model.weights.w_perfold');
% % % 
% % %     [my_ste,t,n_in_column,p,m] = ste(STATS_FINAL.full_model.weights.w_perfold');
% % %     Z = (m ./ mystd)';
% % % 
% % %     iimg_reconstruct_vols(m(1: size_orig{i}(2))', maskInfo, 'outname', 'xval_bestsubsets_wts_mean.img');
% % %     iimg_reconstruct_vols(Z(1: size_orig{i}(2)), maskInfo, 'outname', 'xval_bestsubsets_Z.img');
% % % 
% % %     if length(m) > size_orig{i}
% % %         iimg_reconstruct_vols(m(size_orig{i}(2)+1:end)', maskInfo, 'outname', 'xval_bestsubsets_wts_mean_imageset2.img');
% % %         iimg_reconstruct_vols(Z(size_orig{i}(2)+1:end), maskInfo, 'outname', 'xval_bestsubsets_Z_imageset2.img');
% % %     end
% % % 
% % %     sig_vox = abs(Z) > bonf_thresh;
% % %     Z_thresh = Z;
% % %     Z_thresh(~sig_vox) = 0;
% % %     iimg_reconstruct_vols(Z_thresh(1: size_orig{i}(2)), maskInfo, 'outname', 'xval_bestsubsets_Z_bonf_thresh.img');
% % %     
% % %     if length(m) > size_orig{i}
% % %         iimg_reconstruct_vols(Z_thresh(size_orig{i}(2)+1:end), maskInfo, 'outname', 'xval_bestsubsets_Z_bonf_thresh_imageset2.img');
% % %     end
% % % 
% % %     %% Orthviews
% % %     % -------------------------------------------------------------------------
% % % 
% % %     if any(Z_thresh)
% % %         cl = mask2clusters('xval_bestsubsets_Z_bonf_thresh.img');
% % %     else
    ...
    if dononparam
        ...
        if ~isempty(covs)
            ...
            for i = 1:niter
                ...
                % ------------------------------------------------------------------
                if size(covs, 2) > 1
                    STATSpcovs = xval_regression_multisubject('bestsubsets', my_outcomesp, covs, 'noverbose',  'holdout_method', holdout_method); %,'pca', 'ndims', ndims);
                else
        ...
        if size(dat{1}, 2) > n_vox
            ...
            for i = 1:niter
                ...
                %% Do the prediction
                % ------------------------------------------------------------------
                STATSpv= xval_regression_multisubject('bestsubsets', my_outcomesp, my_datp, 'pca', plsstr, 'ndims', ndims, dochoose_regparams_str,  'holdout_method', holdout_method, 'noverbose');
        ...
        for i = 1:niter
            ...
            %% Do the prediction
            % ------------------------------------------------------------------
            STATSp = xval_regression_multisubject('bestsubsets', my_outcomesp, dat, 'pca', plsstr, 'ndims', ndims,  dochoose_regparams_str, 'holdout_method', holdout_method);
...
% %         ix2 = randperm(size(dat{s}, 2));
% %
% %         STATSperm= xval_regression_multisubject('bestsubsets', {my_outcomes{1}(ix)}, {dat{1}(:, ix2(1:n_vox(i)))}, 'pca', 'ndims', ndims);
% %
...
% %     %ix2 = randperm(size(dat{s}, 2));
% %
% %     STATSperm= xval_regression_multisubject('bestsubsets', {my_outcomes{1}(ix)}, dat(1), 'pca', 'ndims', ndims);
% %
% %     all_r_perm(j, i) = STATSperm.r_each_subject;
% %
% % end
