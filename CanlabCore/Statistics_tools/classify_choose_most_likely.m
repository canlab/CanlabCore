...
%
% Given : 
%   ptask, likelihood of each task given activation, p(task | activation)
%           classses x variables (features, brain voxels)
%
%   testvec, activation values across features
%           variables x 1
%
% Return:
%   taskclass: integer for which is max likelihood class
%   maxlike : log likelihood of chosen class given data (if testvec is 1/0
%               indicator)
%   likeratio : likelihood ratio for most likely vs. least likely class
%
% Example:
% whsave = sum(indx > 0, 2) > 0;    % voxels in original image space that
% were in dataset
%
