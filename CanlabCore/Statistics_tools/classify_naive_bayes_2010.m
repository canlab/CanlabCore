function varargout = classify_naive_bayes_2010(meth, varargin)
...
% %         volInfo.xyzlist(~whkeep',:) = [];
        ...
        for i = 1:nobs
            ...
            if do_feature_abstract
                [reducedY, dummy, cluster_indx] = bayes_meta_feature_abstract(Yorig(include_in_training, :), .1, .1, bayes_model, volInfo, 0);
...
if return_full_ps
    ...
    % crossvalidation.
    [priors, pa1_given_t, pa0_given_t] = bayes_get_probabilities_2010(Y, Xi, k);
...
%[priors, pa1_given_t, pa0_given_t, varargout] = bayes_get_probabilities_2010(Y, Xi, bayes_model.params.k);
...
% now done in bayes_get_probabilities_2010
