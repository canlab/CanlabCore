...
% load_image_set / annotate_binary_results_map handle missing files by
% printing "CANNOT FIND IMAGES ..." or "Find and add the file ..." with
% disp() and then aborting via `error('Exiting')` - so ME.message is
% literally "Exiting" or "Exiting." with an empty identifier.
bare_exiting = isempty(ME.identifier) && ...
               ismember(strtrim(strrep(ME.message, '.', '')), ...
                        {'Exiting', 'exiting'});
