...
%    'xlabelslant' : followed by number of degrees to slant bar labels
%                    (DEFAULT: 45)
...
if ~exist('xlabels','var')
    ...
    xaxis_labeling(xl,xlabels,xlabelslant)
end
...
function xaxis_labeling(xaxislabel,ticklabels,slant)
...
if slant == 0
    ...
    if ~isempty(xaxislabel),
        ...
        t2 = text(XMidPoint,LowYPoint,xaxislabel,'VerticalAlignment','top','HorizontalAlignment','center','Interpreter','none');
        set(t2,'FontSize',18);
        t2ex = get(t2,'Extent');
    else
        t2ex = [0 0];
