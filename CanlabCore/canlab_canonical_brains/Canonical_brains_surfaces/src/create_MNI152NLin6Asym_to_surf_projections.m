...
% template white
create_canlab_surf_file(fullfile('templates','MNI152NLin6Asym_white_lh.surf.gii'), 'MNI152NLin6Asym_white_lh.mat');
create_canlab_surf_file(fullfile('templates','MNI152NLin6Asym_white_rh.surf.gii'), 'MNI152NLin6Asym_white_rh.mat');

% template sphere
create_canlab_surf_file(fullfile('templates','MNI152NLin6Asym_sphere.reg_lh.surf.gii'), 'MNI152NLin6Asym_sphere.reg_lh.mat');
create_canlab_surf_file(fullfile('templates','MNI152NLin6Asym_sphere.reg_rh.surf.gii'), 'MNI152NLin6Asym_sphere.reg_rh.mat');
...
src_img = fmri_data(atlas_obj.select_atlas_subset('G_postcentral').threshold(0.2));
tic;
surf_white_lh = load(which('MNI152NLin6Asym_white_lh.mat'),'faces','vertices');
[~,~,mesh_struct] = reconstruct_image(src_img);
c_white_lh = interp3(mesh_struct.X, mesh_struct.Y, mesh_struct.Z, mesh_struct.voldata, ...
            surf_white_lh.vertices(:,1), surf_white_lh.vertices(:,2), surf_white_lh.vertices(:,3));
surf_white_rh = load(which('MNI152NLin6Asym_white_rh.mat'));
[~,~,mesh_struct] = reconstruct_image(src_img);
c_white_rh = interp3(mesh_struct.X, mesh_struct.Y, mesh_struct.Z, mesh_struct.voldata, ...
            surf_white_rh.vertices(:,1), surf_white_rh.vertices(:,2), surf_white_rh.vertices(:,3));
toc
%% freesurfer resampling
tic
sphere_reg_lh = load('/home/bogdan/.matlab/canlab/CanlabCore/CanlabCore/canlab_canonical_brains/Canonical_brains_surfaces/MNI152NLin6Asym_sphere.reg_lh.mat');
...
[c_fs_sphere_lh, BcC_MNI6_to_fsavg_lh, BcV_MNI6_to_fsavg_lh] = spherical_icosahedral_interpolation(sphere_reg_lh.vertices - mean(sphere_reg_lh.vertices), sphere_reg_lh.faces, c_white_lh, freesurfer_sphere_lh.vertices);

sphere_reg_rh = load('/home/bogdan/.matlab/canlab/CanlabCore/CanlabCore/canlab_canonical_brains/Canonical_brains_surfaces/MNI152NLin6Asym_sphere.reg_rh.mat');
...
[c_fs_sphere_rh, BcC_MNI6_to_fsavg_rh, BcV_MNI6_to_fsavg_rh] = spherical_icosahedral_interpolation(sphere_reg_rh.vertices, sphere_reg_rh.faces, c_white_rh, freesurfer_sphere_rh.vertices);
...
[c_hcp_sphere_lh, BcC_fsavg_to_fslr32_lh, BcV_fsavg_to_fslr32_lh]  = spherical_icosahedral_interpolation(fsLR_to_fs_sphere_lh.vertices, fsLR_to_fs_sphere_lh.faces, c_fs_sphere_lh, fsLR_sphere_lh.vertices);
...
[c_hcp_sphere_rh, BcC_fsavg_to_fslr32_rh, BcV_fsavg_to_fslr32_rh] = spherical_icosahedral_interpolation(fsLR_to_fs_sphere_rh.vertices, fsLR_to_fs_sphere_rh.faces, c_fs_sphere_rh, fsLR_sphere_rh.vertices);
toc
...
[c_lh_colored, c_rh_colored] = map_to_colors(c_fs_sphere_lh, c_fs_sphere_rh, o2);
...
[c_lh_colored, c_rh_colored] = map_to_colors(c_hcp_sphere_lh, c_hcp_sphere_rh, o2);
...
weights_lh = zeros(size(BcC_fsavg_to_fslr_lh,1),9);
vertices_lh = zeros(size(BcC_fsavg_to_fslr_lh,1),9);
val_lh = zeros(size(BcC_fsavg_to_fslr_lh,1),1);
for target_vx = 1:size(BcC_fsavg_to_fslr_lh,1)
    MNI_vx = BcV_MNI6_to_fsavg_lh(BcV_fsavg_to_fslr_lh(target_vx,:),:);
    MNI_weights = BcC_MNI6_to_fsavg_lh(BcV_fsavg_to_fslr_lh(target_vx,:),:).*BcC_fsavg_to_fslr_lh(target_vx,:)';
...
weights_rh = zeros(size(BcC_fsavg_to_fslr32_rh,1),9);
vertices_rh = zeros(size(BcC_fsavg_to_fslr32_rh,1),9);
...
[c_lh_colored, c_rh_colored] = map_to_colors(val_lh, val_rh, o2);
...
descrip = 'this struct provides vertices indexed into CanlabCore/canlab_canonical_brains/Canonical_brains_surfaces/MNI152NLin6Asym_sphere.reg_lh.mat (a freesurfer generated surface reconstruction of MNI152NLin6Asym) and associated weights to remap vertices into HCP''s fs_LR 32k template space.';
resample_from_MNI152NLin6Asym_to_fsavg_164k = struct('weights_lh', BcC_MNI6_to_fsavg_lh, 'vertices_lh', BcV_MNI6_to_fsavg_lh, 'weights_rh', BcC_MNI6_to_fsavg_rh, 'vertices_rh', BcV_MNI6_to_fsavg_rh, 'description', descrip);

descrip = 'this struct provides vertices indexed into CanlabCore/canlab_canonical_brains/Canonical_brains_surfaces/MNI152NLin6Asym_sphere.reg_lh.mat (a freesurfer generated surface reconstruction of MNI152NLin6Asym) and associated weights to remap vertices into HCP''s fs_LR 32k template space.';
resample_from_MNI152NLin6Asym_to_fsLR_32k = struct('weights_lh', weights_lh, 'vertices_lh', vertices_lh, 'weights_rh', weights_rh, 'vertices_rh', vertices_rh, 'description', descrip);

save('/home/bogdan/.matlab/canlab/CanlabCore/CanlabCore/canlab_canonical_brains/Canonical_brains_surfaces/resample_from_MNI152NLin6Asym_to_fsLR_32k.mat', 'resample_from_MNI152NLin6Asym_to_fsLR_32k');
save('/home/bogdan/.matlab/canlab/CanlabCore/CanlabCore/canlab_canonical_brains/Canonical_brains_surfaces/resample_from_MNI152NLin6Asym_to_fsavg_164k.mat', 'resample_from_MNI152NLin6Asym_to_fsavg_164k');