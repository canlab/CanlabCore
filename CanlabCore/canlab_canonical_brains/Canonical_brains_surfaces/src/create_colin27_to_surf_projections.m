...
% but we don't have them in colin27 space. Shen however is in colin27 space.
...
% template white
create_canlab_surf_file(fullfile('templates','colin27_white_lh.surf.gii'), 'colin27_white_lh.mat');
create_canlab_surf_file(fullfile('templates','colin27_white_rh.surf.gii'), 'colin27_white_rh.mat');

% template sphere
create_canlab_surf_file(fullfile('templates','colin27_sphere.reg_lh.surf.gii'), 'colin27_sphere.reg_lh.mat');
create_canlab_surf_file(fullfile('templates','colin27_sphere.reg_rh.surf.gii'), 'colin27_sphere.reg_rh.mat');
...
src_img = fmri_data(atlas_obj.select_atlas_subset(33));
tic;
surf_white_lh = load(which('colin27_white_lh.mat'),'faces','vertices');
...
surf_white_rh = load(which('colin27_white_rh.mat'));
[~,~,mesh_struct] = reconstruct_image(src_img);
...
tic
sphere_reg_lh = load('/home/bogdan/.matlab/canlab/CanlabCore/CanlabCore/canlab_canonical_brains/Canonical_brains_surfaces/colin27_sphere.reg_lh.mat');
...
[c_fs_sphere_lh, BcC_c27_to_fsavg_lh, BcV_c27_to_fsavg_lh] = spherical_icosahedral_interpolation(sphere_reg_lh.vertices - mean(sphere_reg_lh.vertices), sphere_reg_lh.faces, c_white_lh, freesurfer_sphere_lh.vertices);

sphere_reg_rh = load('/home/bogdan/.matlab/canlab/CanlabCore/CanlabCore/canlab_canonical_brains/Canonical_brains_surfaces/colin27_sphere.reg_rh.mat');
...
[c_fs_sphere_rh, BcC_c27_to_fsavg_rh, BcV_c27_to_fsavg_rh] = spherical_icosahedral_interpolation(sphere_reg_rh.vertices, sphere_reg_rh.faces, c_white_rh, freesurfer_sphere_rh.vertices);
...
weights_lh = zeros(size(BcC_fsavg_to_fslr32_lh,1),9);
vertices_lh = zeros(size(BcC_fsavg_to_fslr32_lh,1),9);
val_lh = zeros(size(BcC_fsavg_to_fslr32_lh,1),1);
for target_vx = 1:size(BcC_fsavg_to_fslr32_lh,1)
    MNI_vx = BcV_c27_to_fsavg_lh(BcV_fsavg_to_fslr32_lh(target_vx,:),:);
    MNI_weights = BcC_c27_to_fsavg_lh(BcV_fsavg_to_fslr32_lh(target_vx,:),:).*BcC_fsavg_to_fslr32_lh(target_vx,:)';
...
for target_vx = 1:size(BcC_fsavg_to_fslr32_rh,1)
    MNI_vx = BcV_c27_to_fsavg_rh(BcV_fsavg_to_fslr32_rh(target_vx,:),:);
    MNI_weights = BcC_c27_to_fsavg_rh(BcV_fsavg_to_fslr32_rh(target_vx,:),:).*BcC_fsavg_to_fslr32_rh(target_vx,:)';
...
descrip = 'this struct provides vertices indexed into CanlabCore/canlab_canonical_brains/Canonical_brains_surfaces/colin27_sphere.reg_lh.mat (a freesurfer generated surface reconstruction of colin27) and associated weights to remap vertices into HCP''s fs_LR 32k template space.';
resample_from_colin27_to_fsavg_164k = struct('weights_lh', BcC_c27_to_fsavg_lh, 'vertices_lh', BcV_c27_to_fsavg_lh, 'weights_rh', BcC_c27_to_fsavg_rh, 'vertices_rh', BcV_c27_to_fsavg_rh, 'description', descrip);

descrip = 'this struct provides vertices indexed into CanlabCore/canlab_canonical_brains/Canonical_brains_surfaces/colin27_sphere.reg_lh.mat (a freesurfer generated surface reconstruction of colin27) and associated weights to remap vertices into HCP''s fs_LR 32k template space.';
resample_from_colin27_to_fsLR_32k = struct('weights_lh', weights_lh, 'vertices_lh', vertices_lh, 'weights_rh', weights_rh, 'vertices_rh', vertices_rh, 'description', descrip);

save('/home/bogdan/.matlab/canlab/CanlabCore/CanlabCore/canlab_canonical_brains/Canonical_brains_surfaces/resample_from_colin27_to_fsLR_32k.mat', 'resample_from_colin27_to_fsLR_32k');
save('/home/bogdan/.matlab/canlab/CanlabCore/CanlabCore/canlab_canonical_brains/Canonical_brains_surfaces/resample_from_colin27_to_fsavg_164k.mat', 'resample_from_colin27_to_fsavg_164k');