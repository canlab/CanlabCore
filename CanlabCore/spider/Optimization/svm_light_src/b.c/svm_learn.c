     ...
     /* model:       Returns learning result (assumed empty before called) */
{
  ...
  init_shrink_state(&shrink_state,totdoc,(long)20000);
  ...
  learn_parm->eps=-1.0;      /* equivalent regession epsilon for
				classification */
  ...
  for(i=0;i<totdoc;i++) {    /* various inits */
	...
	fabs(class[i]);
      ...
      learn_parm->svm_cost[i]=learn_parm->svm_c*fabs(class[i]);
      label[i]=-1;
      trainneg++;
    }
    else {
     ...
     /* model:       Returns learning result (assumed empty before called) */
{
  ...
  for(i=0;i<totdoc;i++) {   
    ...
    label[i]=+1;
    c[i]=value[i];
    docs[2*totdoc-1-i]=docs_org[i];
    docs[2*totdoc-1-i].docnum=2*totdoc-1-i;
    label[2*totdoc-1-i]=-1;
    c[2*totdoc-1-i]=value[i];
  }
  totdoc*=2;
