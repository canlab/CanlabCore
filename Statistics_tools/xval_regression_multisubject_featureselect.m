function STATS = xval_regression_multisubject(fit_method, Y, X, varargin)
    ...
    for s = 1:N
        ...
        for wh_fold = 1:length(holdout_set)  %holdout_set = 1:length(Y{s})
            ...
            pthreshold = .05;
            [r, p, Tstat] = correlation_fast_series(train_dat, train_y);
            ...
            train_dat = train_dat(:, wh_features);
            test_dat = test_dat(:, wh_features);
...
% %         end
    ...
    function nested_output_metrics_and_plots
        ...
        if verbose || verboseL
            ...
            plot(Y_orig{s}, 'o-');
            hold on; plot(fit, 'rx-');
            title('Prediced (red), Actual (blue) over obs');
