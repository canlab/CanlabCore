...
The figures in this walkthrough are generated by the scripts in [`_gen/`](_gen/) (`gen_02_montages.m`, `gen_03_surfaces.m`, …), which write PNGs to [`figures/`](figures/) via the helper `wt_save.m`. Run any of them in MATLAB with CanlabCore on the path to regenerate that section's figures.
